...
#  http://ronluna.mooo.com:3480/data_request?id=lr_LuaTest&list=values   right value
...
def toggle(device, vera_controller):
    ...
    if device['category'] == category.BINARY_LIGHT:
        ...
        if int(device['status']):
            newTargetValue = '0'
        vera_controller.GET( \
'/data_request?id=lu_action&DeviceNum=' + str(device['id'])                    + \
...
'&action=SetModeTarget&NewModeTarget=' + mode
        print "mode update :" + command
        vera_controller.GET(command)
